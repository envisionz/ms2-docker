...
RUN mkdir -p mapstore-bin && cd mapstore-bin \
    ...
    && rm -rf ./mapstore-bin \
    && curl -L -o ../ms2-dist.zip https://github.com/envisionz/MapStore2/releases/download/rel-test5-envisionz/frontend-dist.zip \
    && unzip -o ../ms2-dist.zip \
    && rm ../ms2-dist.zip

RUN curl -L -o /frontend-source-maps.zip \
        https://github.com/envisionz/MapStore2/releases/download/rel-test5-envisionz/frontend-source-maps.zip
